      ...
      try {
        this.homeWin.webContents.executeJavaScript(`
          if (typeof initializeHomeWindow === 'function') {
            initializeHomeWindow().then(() => {
              console.log('[home-init] Home window initialized');
            }).catch(err => console.error('Failed to initialize home window:', err));
          }
        `).catch(err => console.warn('[window-manager] executeJavaScript failed:', err.message));
      } catch (err) {
        console.warn('[window-manager] Could not trigger home.js lazy load:', err.message);
      }
    });

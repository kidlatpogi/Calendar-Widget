...
fetchText(url, (err, ics) => {
  ...
  const hci = matches.filter(m => /SUMMARY:Introduction to Human Computer Interaction/i.test(m));
  console.log('VEVENT count for HCI:', hci.length);
  hci.forEach((b, i) => {
    console.log('--- VEVENT', i+1, '---');
    b.split(/\r?\n/).forEach(l => {
      if (/^\s*(UID|RECURRENCE-ID|DTSTART|SUMMARY)\b/i.test(l)) console.log(l);
    });
  });
});

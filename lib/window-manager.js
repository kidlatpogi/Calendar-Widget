...
class WindowManager {
  ...
  createMainWindow() {
    ...
    // Determine target display to ensure window fits on the correct monitor
    let targetDisplay;
    if (windowPos.x !== undefined && windowPos.y !== undefined) {
      // Use a dummy size to find the display matching the saved position
    ...
    // Pass collapsed state via query parameter to avoid FOUC (Flash of Uncollapsed Content)
    const isCollapsed = cfg.ui?.collapsed === true;
    this.win.loadFile(path.join(__dirname, '..', 'HTML', 'index.html'), { query: { collapsed: isCollapsed ? 'true' : 'false' } });
  ...
  toggleClickThrough() {
    try {
      ...
      // notify renderers about updated config - but only send clickThrough property
      // to prevent unwanted syncing of other properties like collapsed
      const sendUpdate = (w) => {
        if (w && !w.isDestroyed()) {
          // Send a minimal update with only the changed property to avoid side effects
          w.webContents.send('config-updated', { ui: { clickThrough: next } });
        }
      };
      sendUpdate(this.win);
  ...
  setupIpcHandlers() {
    ...
    ipcMain.handle('set-config', async (ev, partial) => {
      const cfg = this.cfgManager.config;
      cfg.ui = cfg.ui || {};
      // Preserve existing collapsed state if not being updated
      const wasCollapsed = cfg.ui.collapsed;
      Object.assign(cfg.ui, partial);
      // If collapsed wasn't in the partial update, preserve the existing value
      if (!partial.hasOwnProperty('collapsed') && typeof wasCollapsed === 'boolean') {
        cfg.ui.collapsed = wasCollapsed;
      }
      this.cfgManager.saveConfig();
      ...
      // Send full config update - this is safe because set-config is called with explicit properties
      ...
      try {
        ...
        try {
          ...
          if (targetWin && !targetWin.isDestroyed()) {
            ...
            const disp = screen.getDisplayMatching(winBounds);
    ...
    // Toggle collapse state (for keyboard shortcut)
    ipcMain.handle('toggle-collapse', (event) => {
      try {
